...
We present \sysname, a transparent middleware layer that sits between LLM agents
and ClickHouse OLAP databases. \sysname introduces three mechanisms:
(1)~\emph{intent-based access control} that parses SQL ASTs to enforce policies
at the semantic level (\eg, allowing aggregations over PII columns while
blocking row-level access);
(2)~\emph{session-based query learning} that persists agent sessions as
embeddings and surfaces relevant past queries to new agents with similar goals;
and
(3)~a curated \emph{explore endpoint} that provides schema metadata, column
semantics, statistics, and join recommendations, eliminating the redundant
schema-discovery queries that dominate agentic workloads.

We evaluate \sysname on two benchmark tracks.
A \emph{security benchmark} (5 attack scenarios, 200 probes) shows that the
middleware blocks 67 additional attacks, reducing PII leakage from 300 to 0
rows.
An \emph{agent-efficiency benchmark} (8 analytical tasks, 3 runs, Qwen-3.5)
reduces total queries by 49\% and eliminates all 106 schema-exploration queries,
though token consumption increases 186\% due to retry loops on policy-blocked
queries---a limitation we discuss and scope to small local models.

\todo{Finalize abstract once all results are confirmed.}
\end{abstract}
...
This shift exposes two critical gaps in current OLAP architectures.
...
\medskip
We present \sysname, a transparent middleware layer that addresses both gaps
without modifying the underlying OLAP engine. \sysname contributes three
mechanisms:
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Intent-based access control} (\S\ref{sec:access}): SQL AST
        ...
        intent and enforces policies at the semantic level.

  \item \textbf{Session-based query learning} (\S\ref{sec:learning}):
        Agent sessions are embedded and persisted in
        ChromaDB~\cite{chromadb2023}, enabling cross-agent knowledge transfer
        for agents with similar analytical goals.

  \item \textbf{Curated explore endpoint} (\S\ref{sec:explore}):
        Access-controlled schema metadata, statistics, sample queries, and
        join recommendations eliminate redundant exploration.
\end{enumerate}

\sysname deploys as a ClickHouse-compatible HTTP proxy, requiring zero changes
to existing agent tooling (\eg, \texttt{clickhouse\_connect}). We evaluate it
on a security benchmark (5 attack scenarios, 200 probes) and an agent
efficiency benchmark (8 analytical tasks, Qwen-3.5), demonstrating significant
improvements in both dimensions.

\todo{Add forward references to specific result highlights.}
